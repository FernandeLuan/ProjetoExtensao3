...
import { usuarioEhAdmin, podeUsarVisaoBarbearia } from "../shared/permissoes.js?v=11.0";
...
function preaquecerHistorico() {
    const executar = () => { void importar("historico", "../shared/historico.js?v=11.0").catch(() => null); };
...
export function preloadInicio() { return importar("registrar", "../shared/registrar.js?v=11.0"); }

async function carregar(targetId) {
    switch (targetId) {
        case "registrar": { const m = await importar("registrar", "../shared/registrar.js?v=11.0"); await m.initRegistrar?.(); await m.abrirRegistrar?.(); return; }
        case "painelFinanceiro": { const m = await importar("painel", "../shared/painel.js?v=11.0"); await m.abrirPainelHoje?.(); return; }
...
function iniciarCarregamento(targetId, section) {
    ...
    const token = iniciarLoadingTela(MENSAGENS[targetId] || "Carregando...", { delay: 420 });
