...
import { aplicarPermissoesInterface, usuarioEhAdmin, podeUsarVisaoBarbearia, podeUsarVisaoProfissional } from "../shared/permissoes.js?v=9.0";
...
document.body.dataset.srnkVisao='barbearia'; document.body.dataset.srnkArea='admin';
...
async function iniciar(user){const contexto=await inicializarContexto(user);if(contexto.perfil?.trocarSenha===true)await exigirTrocaSenhaPrimeiroAcesso(contexto);if(!(usuarioEhAdmin()&&podeUsarVisaoBarbearia())){if(podeUsarVisaoProfissional()){window.location.replace('../profissional/');return;}const e=new Error('Esta conta não possui acesso administrativo.');e.code='SEM_ACESSO';throw e;}aplicarPermissoesInterface();definirConfiguracoes(await carregarConfiguracoesDoBanco());configurarNavegacao();await abrirInicio();appInicializado=true;await finalizarBoot();}
onAuthStateChanged(auth,async user=>{if(!user){window.location.replace('../login.html?destino=admin');return;}if(appInicializado)return;try{await iniciar(user);}catch(error){console.error('[Admin] Falha ao iniciar:',error);if(error?.code==='ACESSO_DESATIVADO'||error?.code==='SEM_ACESSO'){await signOut(auth);window.location.replace(`../login.html?destino=admin&motivo=${error.code==='ACESSO_DESATIVADO'?'desativado':'sem-acesso'}`);return;}await finalizarBoot();mostrarErro('Não foi possível carregar a gestão da barbearia. Confira sua conexão e tente novamente.');}});
