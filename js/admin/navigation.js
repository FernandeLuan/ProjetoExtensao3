...
import { state, onStateChange } from "../shared/state.js?v=12.0";
import { podeUsarVisaoProfissional } from "../shared/permissoes.js?v=12.0";
...
function preaquecerHistorico() { const executar = () => { void importar("historico", "../shared/historico.js?v=12.0").catch(() => null); }; if ("requestIdleCallback" in window) window.requestIdleCallback(executar, { timeout: 1800 }); else window.setTimeout(executar, 900); }
...
export function preloadInicio() { return importar("barbeariaHome", "../shared/barbearia-home.js?v=12.0"); }
async function carregar(targetId) {
    switch (targetId) {
        case "barbeariaHome": { const m = await importar("barbeariaHome", "../shared/barbearia-home.js?v=12.0"); await m.abrirVisaoGeralBarbearia?.(); return; }
        case "historico": { const m = await importar("historico", "../shared/historico.js?v=12.0"); await m.abrirHistoricoHoje?.(); return; }
        case "equipe": { const m = await importar("equipe", "../shared/equipe.js?v=12.0"); m.initEquipe?.(); await m.abrirEquipe?.(); return; }
        ...
        case "configuracoes": { const [c, r] = await Promise.all([importar("configuracoes", "../shared/configuracoes.js?v=12.0"), importar("retroativo", "../shared/retroativo.js?v=12.0")]); c.initConfiguracoes?.(); await r.initRetroativo?.(); await r.prepararRetroativoParaUso?.(); return; }
    }
}
