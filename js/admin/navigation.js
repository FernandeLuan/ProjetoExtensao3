...
import { state, onStateChange } from "../shared/state.js?v=11.0";
import { podeUsarVisaoProfissional } from "../shared/permissoes.js?v=11.0";
import { mostrarErro } from "../shared/services/feedback-service.js?v=11.0";
import { iniciarLoadingTela, finalizarLoadingTela } from "../shared/services/ui-loading-service.js?v=11.0";
import { limparSessaoArea } from "../shared/auth-area-session.js?v=11.0";
...
export function configurarNavegacao() {
    ...
    const voltar = podeUsarVisaoProfissional() ? '<li><a class="menu-area-switch" href="../profissional/login.html?trocar=1"><i class="fas fa-user"></i><span>Área profissional</span></a></li>' : "";
...
function preaquecerHistorico() { const executar = () => { void importar("historico", "../shared/historico.js?v=11.0").catch(() => null); }; if ("requestIdleCallback" in window) window.requestIdleCallback(executar, { timeout: 1800 }); else window.setTimeout(executar, 900); }
...
export function preloadInicio() { return importar("barbeariaHome", "../shared/barbearia-home.js?v=11.0"); }
async function carregar(targetId) {
    switch (targetId) {
        case "barbeariaHome": { const m = await importar("barbeariaHome", "../shared/barbearia-home.js?v=11.0"); await m.abrirVisaoGeralBarbearia?.(); return; }
        case "historico": { const m = await importar("historico", "../shared/historico.js?v=11.0"); await m.abrirHistoricoHoje?.(); return; }
        case "equipe": { const m = await importar("equipe", "../shared/equipe.js?v=11.0"); m.initEquipe?.(); await m.abrirEquipe?.(); return; }
        case "relatorios": { const m = await importar("relatorios", "../shared/relatorios.js?v=11.0"); await m.initRelatorios?.(); await m.prepararRelatoriosHoje?.(); return; }
        case "estoque": { const m = await importar("estoque", "../shared/estoque.js?v=11.0"); m.initEstoque?.(); await m.abrirEstoque?.(); return; }
        case "despesas": { const m = await importar("despesas", "../shared/despesas.js?v=11.0"); m.initDespesas?.(); await m.abrirDespesasAtual?.(); return; }
        case "conta": { const m = await importar("conta", "../shared/conta.js?v=11.0"); m.initConta?.(); await m.abrirConta?.(); return; }
        case "configuracoes": { const [c, r] = await Promise.all([importar("configuracoes", "../shared/configuracoes.js?v=11.0"), importar("retroativo", "../shared/retroativo.js?v=11.0")]); c.initConfiguracoes?.(); await r.initRetroativo?.(); await r.prepararRetroativoParaUso?.(); return; }
    }
}
function iniciarCarregamento(targetId, section) { section?.setAttribute("aria-busy", "true"); section?.classList.add("section-module-loading"); if (execucoesSecao.has(targetId)) return execucoesSecao.get(targetId); const token = iniciarLoadingTela(MENSAGENS[targetId] || "Carregando...", { delay: 420 }); const p = carregar(targetId).catch((e) => { console.error(`[Admin] Erro ao carregar ${targetId}:`, e); mostrarErro("Não foi possível abrir esta seção. Tente novamente."); throw e; }).finally(() => { finalizarLoadingTela(token); execucoesSecao.delete(targetId); section?.removeAttribute("aria-busy"); section?.classList.remove("section-module-loading"); }); execucoesSecao.set(targetId, p); return p; }
export async function exibirSecao(href) { if (!href?.startsWith("#")) return; const targetId = href.slice(1); if (!ORDEM_PADRAO.includes(targetId)) return exibirSecao("#barbeariaHome"); if (secaoAtual === targetId) { window.scrollTo({ top: 0, behavior: "smooth" }); return; } const target = document.querySelector(href); if (!target) return; document.querySelectorAll("main > section").forEach((s) => { s.style.display = "none"; }); target.style.display = "block"; window.scrollTo({ top: 0, behavior: "auto" }); animar(target); secaoAtual = targetId; atualizarAtivo(targetId); return iniciarCarregamento(targetId, target); }
...
export function initNavigation() { if (inicializado) return; inicializado = true; menuToggle?.addEventListener("click", () => sidebarMenu?.classList.contains("active") ? fecharMenu() : abrirMenu()); sidebarOverlay?.addEventListener("click", fecharMenu); document.addEventListener("keydown", (e) => { if (e.key === "Escape") fecharMenu(); }); document.querySelector(".bottom-nav")?.addEventListener("click", (e) => { const item = e.target.closest(".bottom-nav-item[data-nav-target]"); if (!item) return; e.preventDefault(); fecharMenu(); void exibirSecao(`#${item.dataset.navTarget}`); }); sidebarMenu?.addEventListener("click", (e) => { const link = e.target.closest("a"); if (!link) return; if (link.classList.contains("menu-logout")) { e.preventDefault(); void logout(); return; } const href = link.getAttribute("href"); if (href?.startsWith("#")) { e.preventDefault(); void exibirSecao(href); fecharMenu(); } }); onStateChange("configSistema", configurarNavegacao); }
